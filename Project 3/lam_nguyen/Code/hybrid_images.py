...
### Acknowledgment: https://pages.stat.wisc.edu/~mchung/teaching/MIA/reading/diffusion.gaussian.kernel.pdf.pdf
...
def low_pass(image, kernel):
    return scipy.signal.convolve2d(image, kernel, mode='same')

def high_pass(image, kernel):
    return image - low_pass(image, kernel)

def hybrid_image(im1, im2):
    # Generate gaussian kernelm
    kernal = gaussian_kernel()

    # Get the low and high pass
    return low_pass(im1, kernal), high_pass(im2, kernal)
...
if __name__ == "__main__":
    ...
    im_low, im_high = hybrid_image(im2_aligned, im1_aligned)
    im = im_low + im_high
    #im = im / im.max()
